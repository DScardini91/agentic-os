...
| `runtime` | Execution backend: `claude`, `codex`, or `legacy/unknown`. Never infer absent runtime as a named backend — tag as `legacy/unknown`. Used to segment governance metrics and surface backend-specific enforcement patterns in multi-backend deployments. |
...
Every enforcement trigger starts at minimum severity and earns each escalation through evidence. Three-phase lifecycle:

| Phase | State | Promotion condition |
|-------|-------|---------------------|
| 1 | **log-only** | Probe deployed; records accumulating; caller attribution not yet reliable |
| 2 | **warn-only** | ≥100 fresh records in scope + ≥95% reliable caller attribution + manual false-positive sample reviewed + Senior Advisor approval |
| 3 | **hard-block** | 7-day warn-only window with zero confirmed false positives + Senior Advisor approval + decision-log entry |

> Premature hard-blocks cause false-positive enforcement that erodes operator trust. Caller attribution reliability is a prerequisite for identity-based enforcement — promoting before attribution is reliable produces blocks that fire on the wrong caller. The gate protocol makes each severity level earned, not assumed.
